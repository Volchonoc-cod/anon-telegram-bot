...
from app.keyboards import main_menu, message_actions_keyboard, recreate_link_keyboard
...
def create_user(db: Session, telegram_id: int, username: str, first_name: str, last_name: str = None):
    user = User(
...
# Обработчик "👁️ Раскрыть отправителя"
...
# Обработчик перехода по анонимной ссылке
...
async def handle_anon_link(message: Message, state: FSMContext):
    ...
    try:
        ...
        current_user = get_user(db, message.from_user.id)
...
# Обработчик текстовых сообщений (отправка анонимного сообщения) - ИСПРАВЛЕННЫЙ
...
async def start_reply(callback: CallbackQuery, state: FSMContext):
    ...
    try:
        ...
        current_user = get_user(db, callback.from_user.id)
        if not current_user:
            await callback.answer("❌ Пользователь не найден")
            return

        # Проверяем, что текущий пользователь - получатель исходного сообщения
        if current_user.id != original_message.receiver_id:
            await callback.answer("❌ Вы не можете ответить на это сообщение")
            return

        # Проверяем, что отправитель исходного сообщения - не текущий пользователь
        ...
        await state.update_data(
            ...
            reply_receiver_id=original_message.sender_id,  # Отвечаем отправителю исходного сообщения
...
# Обработчик текста ответа - ИСПРАВЛЕННЫЙ
...
async def reveal_sender(callback: CallbackQuery):
    ...
    try:
        ...
        # ПРОВЕРКА ВОЗМОЖНОСТИ РАСКРЫТИЯ
